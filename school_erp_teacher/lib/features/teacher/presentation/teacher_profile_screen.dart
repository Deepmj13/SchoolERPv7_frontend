...
import 'package:school_erp_teacher/core/widgets/profile_skeleton_loader.dart';
...
final teacherClassesForProfileProvider = FutureProvider<List<TeacherClass>>((
  ref,
) {
  final teacherId = ref.watch(authStateProvider).user?.teacherId ?? '';
  final repo = ref.watch(teacherRepositoryProvider);
...
final teacherClassTeacherProvider = FutureProvider<ClassModel?>((ref) async {
  final teacherId = ref.watch(authStateProvider).user?.teacherId ?? '';
  final repo = ref.watch(teacherRepositoryProvider);
  try {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            profileAsync.when(
              loading: () =>
                  const ProfileSkeletonLoader(scrollable: false, padding: 0),
              error: (e, _) => Text(
                'Failed to load: $e',
            ...
            if (classTeacherAsync.valueOrNull != null) ...[
              const SizedBox(height: 24),
              Text('My Class', style: Theme.of(context).textTheme.titleLarge),
            ...
            classesAsync.when(
              ...
              data: (classes) {
                ...
                final filtered = classTeacher != null
                    ? classes
                          .where((c) => c.classId != classTeacher.id)
                          .toList()
                    : classes;
                ...
                if (grouped.isEmpty) {
                  return const GlassCard(
                    child: Center(child: Text('No classes assigned')),
                  );
                }
                return Column(
